...
import { fetchPet, fetchdeletePet } from '../../../redux/Slice/PetsManagement';
...
const  OrderDeleteModal: FC<ICustomerEditModalProps> = ({ id, isOpen,deleteId, setIsOpen }) => {
   ...
   const handleDeleteClick =async () => {
    setIsOpen(false);
	try {
		await dispatch(fetchdeletePet(deleteId) as any);
		...
		return (
            ...
            titleId={id.toString()}>
				<ModalHeader setIsOpen={setIsOpen} className='p-4'>
					<ModalTitle id={id}>{item?.name || 'Remove Pet'}</ModalTitle>
				</ModalHeader>
