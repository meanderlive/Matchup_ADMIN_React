	...
	const formik = useFormik({
		...
		onSubmit: async (values: any) => {
			...
			try {
				...
				await dispatch(create_roles({ values }) as any);
				setIsLoading(false);
		...
		return (
			<Modal isOpen={isOpen} setIsOpen={setIsOpen} size='md' titleId={id.toString()}>
				<ModalHeader setIsOpen={setIsOpen} className='p-4'>
					<ModalTitle id={id}>Add Premission</ModalTitle>
				</ModalHeader>
				<ModalBody className='px-4'>
					<div className='row g-4'>
						...
						<FormGroup id='banner_title' label='Banner Title *' className='col-md-12'>
							<Input
								name="banner_title"
								onChange={formik.handleChange}
