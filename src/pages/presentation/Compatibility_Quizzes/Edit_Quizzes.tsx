...
import {   getAllCompatibilityApis, updateCompatibilitySlice } from '../../../redux/Slice/CompatibilitySlice';
...
const Add: FC<ICustomerEditModalProps> = ({ id, isOpen,editId, editData,  setIsOpen }) => {
 ...
 const [answerarray1,setAnswerArray ]= useState<any>(editData?.answer[0] || '')
 const [answerarray2,setAnswerArray2 ]= useState<any>(editData?.answer[1] || '')

 const [answerarray3,setAnswerArray3 ]= useState<any>(editData?.answer[2] || '')

 const [answerarray4,setAnswerArray4 ]= useState<any>(editData?.answer[3] || '')
	...
	const formik = useFormik({	
		...
		onSubmit: async(values) => {
			...
			dispatch(updateCompatibilitySlice({ editId, modify }) as any);
			...
			setTimeout(() => {
				...
				'Quizzes has been Updated successfully',
				);
			}
		},
	});
		...
		return (
			<Modal isOpen={isOpen} setIsOpen={setIsOpen} size='xl' titleId={id.toString()}  >
				<ModalHeader setIsOpen={setIsOpen} className='p-4'>
					<ModalTitle id={id}>{editData?.question || 'Edit Compatibility Quizzes'}</ModalTitle>
				</ModalHeader>
