...
const  DeleteFaq: FC<ICustomerEditModalProps> = ({ id, isOpen,deleteId, setIsOpen }) => {
...
//    setModeid(localStorage.getItem('modeid'))
   const handleDeleteClick = () => {
    setIsOpen(false);
	try {
		dispatch(deleteFaqSlice(deleteId) as any);
		...
		return (
            ...
            titleId={id.toString()}>
				<ModalHeader setIsOpen={setIsOpen} className='p-4'>
					<ModalTitle id={id}>{item?.name || 'New Faaq'}</ModalTitle>
				</ModalHeader>
