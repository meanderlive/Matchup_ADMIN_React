...
// import { delete_Subscription, getAll_Subscription } from '../../../redux/Slice/Subscriptions_Slice';
import { deleteFaqSlice, getallFaqSlice } from '../../../redux/Slice/FaqSlice';
import { deleteInterestSlice, getAllInterest } from '../../../redux/Slice/IntersetSlice';
...
//    setModeid(localStorage.getItem('modeid'))
   const handleDeleteClick = () => {
    setIsOpen(false);
	try {
		dispatch(deleteInterestSlice(deleteId) as any);

			
		}catch (error) {
			
		}finally{
			dispatch(getAllInterest(iddd)as any)
			console.log("sdf")
		}
};
	if (id || id === '0') {
		return (
            ...
            titleId={id.toString()}>
				<ModalHeader setIsOpen={setIsOpen} className='p-4'>
					<ModalTitle id={id}>{item?.name || 'New Interest'}</ModalTitle>
				</ModalHeader>
