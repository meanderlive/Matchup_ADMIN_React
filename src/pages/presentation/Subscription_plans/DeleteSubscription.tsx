...
import { fetchdeleteSubscriptionPlan, getallSubscriptionPlan } from '../../../redux/Slice/SubscriptionPlans_Slice';
...
const  DeleteInterset: FC<ICustomerEditModalProps> = ({ id, isOpen,deleteId, setIsOpen }) => {
...
//    setModeid(localStorage.getItem('modeid'))
   const handleDeleteClick = () => {
    setIsOpen(false);
	try {
		dispatch(fetchdeleteSubscriptionPlan(deleteId) as any);

			
		}catch (error) {
			
		}finally{
			dispatch(getallSubscriptionPlan(iddd)as any)
			console.log("sdf")
		}
};
	if (id || id === '0') {
		return (
            ...
            titleId={id.toString()}>
				<ModalHeader setIsOpen={setIsOpen} className='p-4'>
					<ModalTitle id={id}>{item?.name || 'Delete SubscriptionPlan'}</ModalTitle>
				</ModalHeader>
				<ModalBody className='px-4'>
                <p>Are you sure you want to delete this SubscriptionPlan?</p>
				</ModalBody>
...
DeleteInterset.propTypes = {
	id: PropTypes.string.isRequired,
	isOpen: PropTypes.bool.isRequired,
	setIsOpen: PropTypes.func.isRequired,
};

export default  DeleteInterset;
