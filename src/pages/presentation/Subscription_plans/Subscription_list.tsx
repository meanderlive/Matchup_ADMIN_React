...
const MySubscription = () => {
 ...
 const paramsData = useParams()
	...
	const state = useSelector((statee:any)=>statee.Subscription)
 
	const dataa:any = state?.Subscription?.data
   useEffect(()=>{
	...
	const filteredData = dataa ? dataa.filter((f: any) =>
		f.planName.toLowerCase().includes(formik.values.searchInput.toLowerCase())  
	) : [];
	...
	const handleSearch =(e:any)=>{
		...
		if(inputValue.trim()==='')
		{
			...
			dispatch(fetchsearchSubscription(inputValue)as any)
		}
	}

	const handleSort =(sorrt:any)=>{
    
	 dispatch(fetchsortSubscription(sorrt)as any)
	}
	...
	const handleSortByname=()=>{
		...
		setSort(setName)
		 dispatch(fetchsortSubscription(setName)as any)

	}
 
	return (
		<PageWrapper title={demoPagesMenu.crm.subMenu.customersList.text}>
			<span className='display-5 fw-bold my-3 mx-3 ' >Subscription Plan Management</span>
										 ...
										 <div
											 ...
											 <div
												 ...
												 <span className='fw-bold'>
													 {getFirstLetter(itemss.planName)}
												 </span>
											 </div>
										 </div>
									 </div>
...
<td> {itemss?.planDescription}</td>
<td> {itemss?.planDuration}</td>
<td> {itemss?.planType}</td>
<td> {itemss?.planPrice}</td>
