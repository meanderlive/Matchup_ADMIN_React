...
import { create_Inventorie, getAll_Inventorie } from "../../../redux/Slice/Inventories_Slice";
...
const AddServices: FC<ICustomerEditModalProps> = ({ id, isOpen, setIsOpen }) => {
  ...
  const userValidation = Yup.object().shape({
    ...
    //   .typeError("Initial stock must be a number")
    //   .integer("Initial stock must be an integer")
...
//         formData.append("name", values.product_name.trim());
...
//         formData.append("initial_stock", values.initial_stock.trim());
//         formData.append("service", values.service_id);
...
//         // ✅ Submit FormData
//         const response = await dispatch(create_Inventorie(formData ) as any);
...
const formik = useFormik({
  initialValues: {
    ...
    category: "",
    admin_note: "",
    initial_stock: 0, // Ensure it's a number
    service_id: "",
    is_activated: true,
  },
  validationSchema: userValidation,
  onSubmit: async (values) => {
    ...
    try {
      ...
      console.log("Payload Debugging:", Data);

      // ✅ Submit Object Data
      const response = await dispatch(create_Inventorie({Data}) as any);
      console.log(response, "responseresponse");
            ...
            <div className="row g-4 align-items-center" style={{}}>
              <FormGroup id="product_name" label="Name" className="col-md-6">
                ...
                {errorHandling && formik.errors.product_name ? (
                  <div className="errorMassage">{formik.errors.product_name}</div>
                ) : (
                  <div />
                )}
              </FormGroup>
              ...
              <FormGroup id="category" label="Category" className="col-md-6">
                ...
                {errorHandling && formik.errors.category ? (
                  <div className="errorMassage">{formik.errors.category}</div>
                ) : (
                  <div />
                )}
              </FormGroup>

              <FormGroup id="admin_note" label="Note" className="col-md-12">
                <Input
                  name="admin_note"
                  type="text"
                  onChange={formik.handleChange}
              ...
              <FormGroup id="initial_stock" label="Initial Stock" className="col-md-12">
                <Input
                  name="initial_stock"
                  type="text"
                  onChange={formik.handleChange}
                ...
                {errorHandling && formik.errors.initial_stock ? (
                  <div className="errorMassage">{formik.errors.initial_stock}</div>
                ) : (
                  <div />
                )}
              </FormGroup>
              ...
              <FormGroup id="service_id" label="service" className="col-md-6">
                ...
                {errorHandling && formik.errors.service_id ? (
                  <div className="errorMassage">{formik.errors.service_id}</div>
                ) : (
                  <div />
                )}
              </FormGroup>
