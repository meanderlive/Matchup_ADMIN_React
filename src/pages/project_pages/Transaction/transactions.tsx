...
import Add from "./Add_Transaction";

import Edit_User from "./Edit_Transaction";
import OrderDeleteModal from "./Delete_Transaction";
...
import ActiveUser from "./ActiveTransaction";
...
import { getAll_Plan, upload_Plans_Images } from "../../../redux/Slice/Plans_Slice";
...
const Plans = () => {
  ...
  useEffect(() => {
    ...
    // dispatch(getAll_Plan({}) as any);

    // dispatch(fetchUser({ modeid, SelectedRole }) as any); 
  }, [dispatch, currentPage]);

  const stateUser: any = useSelector((state: any) => state.Plan);
  ...
  const handleImageUpload = async (item: any, file: File) => {
    ...
    try {
      const res = await dispatch(upload_Plans_Images({ id, formData }) as any);
      // dispatch(getAll_Plan({}) as any);
  ...
  const handleSearch = (e: any) => {
    ...
    if (inputValue.trim() === "") {
      // dispatch(getAll_Plan({ SelectedPlan }) as any);
    } else {
  ...
  return (
    <PageWrapper title={demoPagesMenu.crm.subMenu.customersList.text}>
      <div className="row  justify-content-around">
        <div className="display-5 col-md-7 fw-bold my-3 ">Transaction</div>
        <div className="col-md-4">
          {/* <FormGroup>
            <Label htmlFor="role">Change Transaction</Label>
            <Select
              id="plan"
              name="plan"
              ariaLabel="Plans"
