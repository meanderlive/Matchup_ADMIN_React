...
import Add from "./Add_Bookings";

import Edit_User from "./Edit_Bookings";
import OrderDeleteModal from "./Delete_Bookings";
...
import ActiveUser from "./ActiveBookings";
...
import { getAll_Bookings, upload_Booking_Images } from "../../../redux/Slice/Bookings_Slice";
...
const Plans = () => {
  ...
  const handleStatus = async (idedi: any, status: any) => {
    ...
    await dispatch(getAll_Bookings({}) as any);
  };
  ...
  useEffect(() => {
    ...
    dispatch(getAll_Bookings({}) as any);

    // dispatch(fetchUser({ modeid, SelectedRole }) as any);
  }, [dispatch, currentPage]);

  const stateUser: any = useSelector((state: any) => state.Booking);
  ...
  const handleImageUpload = async (item: any, file: File) => {
    ...
    try {
      const res = await dispatch(upload_Booking_Images({ id, formData }) as any);
      dispatch(getAll_Bookings({}) as any);
  ...
  const handleSearch = (e: any) => {
    ...
    if (inputValue.trim() === "") {
      dispatch(getAll_Bookings({ SelectedPlan }) as any);
    } else {
  ...
  return (
    <PageWrapper title={demoPagesMenu.crm.subMenu.customersList.text}>
      <div className="row  justify-content-around">
        <div className="display-5 col-md-7 fw-bold my-3 ">Bookings</div>
        <div className="col-md-4">
          <FormGroup>
            <Label htmlFor="role">Change Bookings</Label>
            <Select
              id="plan"
              name="plan"
              ariaLabel="Plans"
