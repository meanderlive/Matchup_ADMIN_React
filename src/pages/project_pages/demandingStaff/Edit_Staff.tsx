...
import { getAll_demstaffs, update_demstaffs } from '../../../redux/Slice/Staff_Slice ';
...
const EditUser: FC<ICustomerEditModalProps> = ({ id, isOpen, editId, editData, setIsOpen }) => {
	...
	const Service_Data = Array.isArray(store?.Service?.services?.data?.data)
  ? store.Service.services.data?.data
  : [];
	...
	const formik = useFormik({
		...
		onSubmit: async (values: any, { setSubmitting }: FormikHelpers<any>) => {
			...
			try {
				...
				formData.append("admin_note", values?.admin_note || "");
				formData.append("is_activated", values?.is_activated.toString());
				...
				const response = await dispatch(update_demstaffs({ editId, values }) as any);
				...
				// **Refresh plan data**
				await dispatch(getAll_demstaffs({}) as any);
			...
			<Modal isOpen={isOpen} setIsOpen={setIsOpen} size='xl' titleId={id.toString()}>
				...
				<ModalBody className='px-4'>
					<div className='row g-4'>
						...
						<FormGroup id="specialization" label="Specialization" className="col-md-6">
							<Input
								name="specialization"
								type="text"
								onChange={formik.handleChange}
							...
							/>
							{errorHandling && formik.errors.specialization ? (
								<div className="errorMassage">{formik.errors.specialization as any}</div>
							) : (
								<div />
							)}
						</FormGroup>
						<FormGroup id="price" label="Price" className="col-md-6">
							...
							/>
							{errorHandling && formik.errors.price ? (
								<div className="errorMassage">{formik.errors.price as any}</div>
							) : (
								<div />
							)}
						</FormGroup>
						<FormGroup id="other" label="Other" className="col-md-6">
							<Input
								name="other"
								type="text"
								onChange={formik.handleChange}
							...
							/>
							{errorHandling && formik.errors.other ? (
								<div className="errorMassage">{formik.errors.other as any}</div>
							) : (
								<div />
							)}
						</FormGroup>
