...
import Add from "./Add_staff";

import Edit_User from "./Edit_Staff";
import OrderDeleteModal from "./Delete_staff";
...
import ActiveUser from "./Activestaff";
...
import { getAll_demstaffs, upload_demstaffs_Images } from "../../../redux/Slice/Staff_Slice ";
...
const Plans = () => {
  ...
  const Store: any = useSelector((state: any) => state.Dem_Staff);
  const [SelectedPlan, setSelectPlan] = useState(localStorage.getItem("planid") || "")

  const PlanData = Store?.demstaff?.data;
  ...
  useEffect(() => {
    ...
    dispatch(getAll_demstaffs({}) as any);

    // dispatch(fetchUser({ modeid, SelectedRole }) as any); 
  }, [dispatch, currentPage]);

  const stateUser: any = useSelector((state: any) => state.Dem_Staff);
  // console.log(stateUser, "stateUserstateUserstateUserstateUser");

  const reversedDataa = stateUser?.demstaff?.data?.data;
  ...
  const handleImageUpload = async (item: any, file: File) => {
    ...
    try {
      const res = await dispatch(upload_demstaffs_Images({ id, formData }) as any);
      dispatch(getAll_demstaffs({}) as any);
  ...
  const handleSearch = (e: any) => {
    ...
    if (inputValue.trim() === "") {
      dispatch(getAll_demstaffs({ SelectedPlan }) as any);
    } else {
  ...
  return (
    <PageWrapper title={demoPagesMenu.crm.subMenu.customersList.text}>
      <div className="row  justify-content-around">
        <div className="display-5 col-md-7 fw-bold my-3 ">Staff</div>
        <div className="col-md-4">
          <FormGroup>
            <Label htmlFor="role">Change Staff</Label>
            <Select
              id="plan"
              name="plan"
              ariaLabel="Plans"
