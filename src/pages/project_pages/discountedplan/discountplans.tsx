...
import Add from "./Add_DiscountPlans";

import Edit_User from "./Edit_DiscountPlans";
import OrderDeleteModal from "./Delete_DiscountPlans";
...
import ActiveUser from "./ActivePlans";
...
import { getAll_Disconutplan, upload_Disconutplan_Images } from "../../../redux/Slice/DiscountPlans_Slice";
...
const DiscountPlan = () => {
  ...
  const Store: any = useSelector((state: any) => state.Disconut_Plan);
  const [SelectedPlan, setSelectPlan] = useState(localStorage.getItem("planid") || "")

  const PlanData = Store?.Disconutplans?.data;
  console.log(PlanData, "PlanDataPlanDataPlanDataPlanData")
  ...
  const handleStatus = async (idedi: any, status: any) => {
    ...
    await dispatch(getAll_Disconutplan({}) as any);
  };
  ...
  useEffect(() => {
    ...
    dispatch(getAll_Disconutplan({}) as any);

    // dispatch(fetchUser({ modeid, SelectedRole }) as any); 
  }, [dispatch, currentPage]);

  const stateUser: any = useSelector((state: any) => state.Disconut_Plan);
  // console.log(stateUser, "stateUserstateUserstateUserstateUser");

  const reversedDataa = stateUser?.Disconutplans?.data?.data || {};
  console.log(reversedDataa, "reversedDataareversedDataareversedDataareversedDataareversedDataa1111111")
  ...
  const handleImageUpload = async (item: any, file: File) => {
    ...
    try {
      const res = await dispatch(upload_Disconutplan_Images({ id, formData }) as any);
      dispatch(getAll_Disconutplan({}) as any);
  ...
  const handleSearch = (e: any) => {
    ...
    if (inputValue.trim() === "") {
      dispatch(getAll_Disconutplan({ SelectedPlan }) as any);
    } else {
  ...
  return (
    <PageWrapper title={demoPagesMenu.crm.subMenu.customersList.text}>
      <div className="row  justify-content-around">
        <div className="display-5 col-md-7 fw-bold my-3 ">Discounted Plans</div>
        <div className="col-md-4">
          <FormGroup>
            <Label htmlFor="role">Change Plans</Label>
            <Select
              id="plan"
              name="plan"
              ariaLabel="Plans"
