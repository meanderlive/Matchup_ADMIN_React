...
import Add from "./Add_Payment";

import Edit_User from "./Edit_Payment";
import OrderDeleteModal from "./Delete_Payment";
...
import ActiveUser from "./ActivePayments";
...
import { getAll_Payments, upload_Payment_Images } from "../../../redux/Slice/Payments_Slice";
...
const Plans = () => {
  ...
  const handleStatus = async (idedi: any, status: any) => {
    ...
    await dispatch(getAll_Payments({}) as any);
  };
  ...
  useEffect(() => {
    ...
    dispatch(getAll_Payments({}) as any);

    // dispatch(fetchUser({ modeid, SelectedRole }) as any);
  }, [dispatch, currentPage]);

  const stateUser: any = useSelector((state: any) => state.Payment);
  ...
  const handleImageUpload = async (item: any, file: File) => {
    ...
    try {
      const res = await dispatch(upload_Payment_Images({ id, formData }) as any);
      dispatch(getAll_Payments({}) as any);
  ...
  const handleSearch = (e: any) => {
    ...
    if (inputValue.trim() === "") {
      dispatch(getAll_Payments({ SelectedPlan }) as any);
    } else {
  ...
  return (
    <PageWrapper title={demoPagesMenu.crm.subMenu.customersList.text}>
      <div className="row  justify-content-around">
        <div className="display-5 col-md-7 fw-bold my-3 ">Payments</div>
        <div className="col-md-4">
          <FormGroup>
            <Label htmlFor="role">Change payment</Label>
            <Select
              id="plan"
              name="plan"
              ariaLabel="Plans"
