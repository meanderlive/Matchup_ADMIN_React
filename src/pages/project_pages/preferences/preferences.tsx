...
import Add from "./Add_Preferences";

import Edit_User from "./Edit_Preferences";
import OrderDeleteModal from "./Delete_Preferences";
...
import ActiveUser from "./ActivePreferences";
...
import { getAll_Preference, upload_Preferences_Images } from "../../../redux/Slice/Preferences_Slice";
...
const Plans = () => {
  ...
  const handleStatus = async (idedi: any, status: any) => {
    ...
    await dispatch(getAll_Preference({}) as any);
  };
  ...
  const stateUser: any = useSelector((state: any) => state.Preferences);
  console.log(stateUser, "stateUserstateUserstateUserstateUser");

  const reversedDataa = stateUser?.modes?.data;
  ...
  const handleImageUpload = async (item: any, file: File) => {
    ...
    try {
      const res = await dispatch(upload_Preferences_Images({ id, formData }) as any);
      dispatch(getAll_Preference({}) as any);
  ...
  const handleSearch = (e: any) => {
    ...
    if (inputValue.trim() === "") {
      dispatch(getAll_Preference({ SelectedPlan }) as any);
    } else {
  ...
  const PlanOptions = PlanData && PlanData?.map((item: Plan) => ({
    value: item?._id,
    text: item?.name
  }));
  ...
  return (
    <PageWrapper title={demoPagesMenu.crm.subMenu.customersList.text}>
      <div className="row  justify-content-around">
        <div className="display-5 col-md-7 fw-bold my-3 ">Preferences</div>
        <div className="col-md-4">
          <FormGroup>
            <Label htmlFor="role">Change Preferences</Label>
            <Select
              id="plan"
              name="plan"
              ariaLabel="Plans"
