...
import OrderDeleteModals from "./DeleteSubscriptionsPlan";
...
import { getallSubscriptionPlan, SubscriptionPlans_Images } from "../../../redux/Slice/SubscriptionPlans_Slice";
...
const Plans = () => {
  ...
  const Store: any = useSelector((state: any) => state.Sub_Plan);
  console.log(Store,"Sub_PlanSub_PlanSub_Plan") 
  const [SelectedPlan, setSelectPlan] = useState(localStorage.getItem("planid") || "")

  const Sub_PlanData = Store?.SubscriptionPlan?.data?.data || [];
  console.log(Sub_PlanData,"Sub_PlanDataSub_PlanDataSub_PlanData")
  ...
  const handleStatus = async (idedi: any, status: any) => {
    ...
    await dispatch(getallSubscriptionPlan({}) as any);
  };
  ...
  useEffect(() => {
    ...
    dispatch(getallSubscriptionPlan({}) as any);
  ...
  const reversedDataa = Sub_PlanData
  ...
  const handleImageUpload = async (item: any, file: File) => {
    ...
    try {
      const res = await dispatch(SubscriptionPlans_Images({ id, formData }) as any);
      dispatch(getallSubscriptionPlan({}) as any);
  ...
  const handleSearch = (e: any) => {
    ...
    if (inputValue.trim() === "") {
      dispatch(getallSubscriptionPlan({ SelectedPlan }) as any);
    } else {
  ...
  // const PlanOptions = Sub_PlanData && Sub_PlanData?.map((item: Plan) => ({
  //   value: item?._id,
  //   text: item?.name
  // }));
  ...
  return (
    <PageWrapper title={demoPagesMenu.crm.subMenu.customersList.text}>
      <div className="row  justify-content-around">
        <div className="display-5 col-md-7 fw-bold my-3 ">Subscriptions Plan</div>
        <div className="col-md-4">
          <FormGroup>
            <Label htmlFor="role">Change Subscriptions Plan</Label>
            <Select
              id="plan"
              name="plan"
              ariaLabel="Plans"
